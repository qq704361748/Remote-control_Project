...
{
	m_lock            = false;
	m_hCompletionPort = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, NULL,
	                                           1);
...
void CQueue<T>::DealParam(PPARAM* pParam)
{
	...
	case QClear:
		...
		break;
	default: OutputDebugString(TEXT("unknown operator!\r\n"));
	}
}

template <class T>
inline KSendQueue<
	T>::KSendQueue(ThreadFuncBase* obj, KCALLBACK callback): CQueue<T>(),
	m_base(obj), m_callback(callback)
{
	m_thread.Start();
	m_thread.
			UpdateWorker(::ThreadWorker(this, (FUNCTYPE)&KSendQueue<
				                            T>::threadTick));
}
...
{
	typename CQueue<T>::IocpParam* Param = new typename CQueue<
		T>::IocpParam(CQueue<T>::QPop, T());
